...
def generate_gif_from_description(description_text, num_frames=6, image_size="512x512", style="product"):
    ...
    try:
        ...
        for i in range(1, num_frames):
            ...
            frames.append(image)        # Create a GIF with smoother animation
        output_gif_dir = "generated_gifs"
        os.makedirs(output_gif_dir, exist_ok=True)
        
        # Also ensure the static directory exists
        static_gif_dir = os.path.join("static", "generated_gifs")
        os.makedirs(static_gif_dir, exist_ok=True)
        ...
        static_gif_filepath = os.path.join(static_gif_dir, gif_filename)
        try:
            # Copy the file to ensure it's available in both locations
            import shutil
            shutil.copy2(gif_filepath, static_gif_filepath)
            print(f"GIF also copied to static directory: {static_gif_filepath}")
        except Exception as e:
            print(f"Warning: Could not copy GIF to static directory: {e}")
